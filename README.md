**PL-cLab** - это язык программирования для STM32. 
...
- [Основные положения](#основные-положения)
  - [Как писать программу для ITMO.cLab](#как-писать-программу-для-itmoclab)
...
- [Примеры программ](#примеры-программ)
  ...
  - [Функция сравнения чисел типа int](#функция-сравнения-чисел-типа-int)
  - [Функция сравнения строк](#функция-сравнения-строк)
  - [Функция вычисления факториала](#функция-вычисления-факториала)
...
Вся работа языка основана на вызове функций и это единственный способ работы с данными в PL-cLab.

Изначально разрабатывался как Forth-подобный, однако из-за непривычно устаревшего вида текста программы на языке Forth было принято решение привести лексику PL-cLab к более современному виду, напоминающему Kotlin или Python. Внутренняя реализация всё ещё является Forth-подобной, хотя и имеет некоторые дополнения вроде указателей и работы с динамическимвыделением памяти.

## Как писать программу для ITMO.cLab
Для того, чтобы написать свою программу для ITMO.cLab, необходимо:
1. Импортировать код из этого репозитория (только папку compiler)
2. В папке compiler находим файл [*Main.java*](https://github.com/IP-13/PL_for_stm32/blob/main/compiler/src/main/java/com/ip13/Main.java)
3. В переменную `program` вставляем код на языке PL-cLab
4. Запускаем *Main.java*
5. В корне генерируется файл *.plClab
6. Код из этого файла полностью (все 1000+ строк) копируем в Web IDE на ITMO.cLab, билдим и запускаем
...
Также существует тип возвращаемого значения `void`.
...
| Микрокоманда | Код | Описание                                          |
|--------------|-----|---------------------------------------------------|
| **BOOL**     | 200 | обозначение логического типа (bool)               |
...
| **MAIN**     | 999 | точка входа в основной листинг программы                                                  |
...
# Примеры программ
Весь код, который описан ниже, работает корректно и протестирован.

## Первая программа
Поздороваемся с PL-cLab
```
MAIN
print("Hello, PL-cLab!");
```

## Функция сравнения чисел типа int
Создадим и протестируем функцию, которая сравнивает 2 значения типа int и возвращает результат типа bool
```
int_equal(A:int, B:int): bool
    START
        RETURN and(not(less(A, B)), not(greater(A, B)));
    FINISH

MAIN
print(int_equal(10, 10));
```

## Функция сравнения строк
Создадим и протестируем функцию, которая сравнивает 2 значения типа string и возвращает результат типа bool
```
int_equal(A: int, B: int): bool
START
